...
const Editor = ({ onOpenAnnotator, lastOutput }: any) => {
	...
	const [selectedExample, changeSelectedExample] = useState(
		window.localStorage.getItem('customInput') ? 'Custom' : 'Simple Bounding Box'
	)
		...
		<div>
			<div className={c.editBar}>
				...
				<div>
					...
					<Button className="button" disabled={!lastOutput} onClick={() => changeOutputOpen(true)}>
						View Output
					</Button>
					<Button
						className="button"
			...
			<div
				...
				style={currentError ? { border: '2px solid #f00' } : { border: '2px solid #fff' }}
			>
				<div>
					<MonacoEditor
