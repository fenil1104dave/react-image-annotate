...
const Row = ({ header, highlighted, order, classification, area, tags, trash, lock, visible, onClick }) => {
	const classes = useStyles()
...
export default ({ regions, onDeleteRegion, onChangeRegion, onSelectRegion }) => {
	const classes = useStyles()
	return (
		<SidebarBoxContainer
			title="Regions"
			...
			<div className={classes.container}>
				...
				{regions.map((r, i) => (
					<Row
						...
						lock={
							r.locked ? (
								<LockIcon onClick={() => onChangeRegion({ ...r, locked: false })} className="icon2" />
							) : (
								<UnlockIcon onClick={() => onChangeRegion({ ...r, locked: true })} className="icon2" />
							)
						}
						visible={
