...
export default (state, action) => {
	...
	switch (action.type) {
		...
		case 'ADD_POLYGON_POINT': {
			...
			return setIn(state, ['images', currentImageIndex, 'regions', regionIndex], { ...polygon, points })
		}
		case 'MOUSE_MOVE': {
			const { x, y } = action
			...
			switch (state.mode.mode) {
				case 'MOVE_POLYGON_POINT': {
					...
					return setIn(
						state,
						['images', currentImageIndex, 'regions', regionIndex, 'points', pointIndex],
						[x, y]
					)
				}
				case 'MOVE_REGION': {
					...
					return setIn(
						...
						moveRegion(state.images[currentImageIndex].regions[regionIndex], x, y)
					)
				}
				case 'RESIZE_BOX': {
					const {
					...
					const dh = yFree === 0 ? oh : yFree === -1 ? oh + (oy - dy) : Math.max(0, oh + (y - oy - oh))
				...
				case 'DRAW_POLYGON': {
					...
					return setIn(
						state,
						['images', currentImageIndex, 'regions', regionIndex, 'points', region.points.length - 1],
						[x, y]
					)
				}
			}
			...
			if (state.mode) {
				switch (state.mode.mode) {
					case 'DRAW_POLYGON': {
						...
						if (!polygon) break
						state = setIn(state, ['images', currentImageIndex, 'regions', regionIndex], {
							...polygon,
		...
		case 'CHANGE_REGION': {
			...
			const regionIndex = getRegionIndex(action.region)
			if (regionIndex === null) return state
			return setIn(state, ['images', currentImageIndex, 'regions', regionIndex], region)
		}
